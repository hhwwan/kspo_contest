...
# pdi 계산
def compute_pdi(use_df: pd.DataFrame, wish_df: pd.DataFrame, exclude_cols = ['성별', '연령', '사례수', '없다']):
    ...
    # 성별, 연령을 인덱스로 지정
    pdi_df['성별연령'] = wish_df['성별'].astype(str) + "_" + wish_df['연령'].astype(str)
    pdi_df = pdi_df.set_index('성별연령')
...
WISH_KEY = 'processed_data/wish_facilities/wish_gender_age.csv'
