...
# pdi 계산
def compute_pdi(use_df: pd.DataFrame, wish_df: pd.DataFrame, exclude_cols = ['지역', '사례수', '없다']):
    # 컬럼 추출 (지역, 사례수, 없다 제외)
    ...
    # 지역명을 인덱스로 지정
    pdi_df['지역'] = wish_df['지역']
    pdi_df = pdi_df.set_index('지역')
...
WISH_KEY = 'processed_data/wish_facilities/wish_region.csv'
